...
class NeuralNetwork:
    ...
    def local_response_normalize(self, radius=5,
                                 bias=1,alpha=1,beta=0.5,name=None):
        """
        Add a local response normalization
        """
        with self.variable_scope():
            self.z = tf.nn.local_response_normalization(self.z,depth_radius=radius,
                                                  alpha=alpha,
                                                  beta=beta,
                                                  bias=bias)
        return self 
    ...
    def avg_pool(self, ksize=(1, 2, 2, 1), strides=(1, 2, 2, 1), padding='VALID'):
        """
        Add a average pooling
        """
        with self.variable_scope():
            self.z = tf.nn.avg_pool(self.z, ksize, strides, padding)
        return self
     
    def resnet_block(self, shape, strides=(1, 1, 1, 1), padding='SAME'):
        """
        Add a resnet block  
        """
        with self.variable_scope('resnet_block'):
            shape = (shape[0], shape[1],int(self.z.get_shape()[-1]),int(self.z.get_shape()[-1]))
            W, b = self.W_b(shape)
            temp = tf.nn.conv2d(self.z, W, strides=strides, padding="SAME") + b
            temp = tf.nn.relu(temp)
            temp = tf.nn.conv2d(temp, W, strides=strides, padding="SAME") + b
            
            self.z = self.z + temp 
        return self
        
    def single_block(self, shape, strides=(1, 1, 1, 1), padding='SAME'):
        with self.variable_scope('resnet_block'):
            shape = (shape[0], shape[1],int(self.z.get_shape()[-1]),int(self.z.get_shape()[-1]))
            W, b = self.W_b(shape)
    ...
    def crelu(self):
        """
        Add a creLU activation
        """
        with self.variable_scope():
            self.z = tf.nn.crelu(self.z)
        return self
    
    def selu(self):
        """
        Add a seLU activation
        """
        with self.variable_scope():
            self.z = tf.nn.selu(self.z)
        return self
    ...
    def tanh(self):
        ...
        """
        with self.variable_scope():
            self.z = tf.nn.tanh(self.z)
        return self
    
    def relu6(self):
        """
        Add a reLU6 activation
        """
        with self.variable_scope():
            self.z = tf.nn.relu6(self.z)
        return self
...
def make_stocahstic(learning_rate):
    """
    A helper to create adam optimizer
    """
    return tf.train.AdamOptimizer(learning_rate=learning_rate)